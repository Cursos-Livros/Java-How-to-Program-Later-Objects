...
public class Exercise31GuessNumberModification {
    ...
    public static void main(String[] args) {
        ...
        while (gameStatus != Status.WIN) {
            gameStatus = checkStatus(guess, randomNumber, countGuesses);
    ...
    public static Status checkStatus(int guess, int randomNumber,int countGuesses) {
        if (guess != randomNumber) {
            checkGuessAgain(guess, randomNumber);
            return Status.LOST;
        }
        showCountGuessesMessages(countGuesses);
    ...
    public static void showCountGuessesMessages(int countGuesses){
        if(countGuesses < 10){
